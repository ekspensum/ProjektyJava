	...
	<c:forEach items="${visitsByPatientAndStatus }" var="visit"  varStatus="loop">
			<tbody>
				<tr><td>${loop.count}</td><td>${visit.visitDateTime}</td><td>${visit.doctor.firstName }</td><td>${visit.doctor.lastName }</td><td></td><td></td></tr>
					<c:forEach items="${visit.treatments }" var="treatment" varStatus="loop2">
						<tr><td>${loop.count}.${loop2.count}</td><td></td><td></td><td></td><td>${treatment.name}</td><td>${treatment.description}</td></tr>
					</c:forEach>			
			</tbody>
	</c:forEach>
</table>
