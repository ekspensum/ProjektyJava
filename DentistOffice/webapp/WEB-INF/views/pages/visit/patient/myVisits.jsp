	...
	<c:forEach items="${visitsByPatientAndStatus }" var="visit"  varStatus="loop">
			<tbody>
				<tr>
					...
					<td colspan="2"></td>
					<td align="center">
					<c:if test="${visit.status.id == 1 }">
						<button form="myVisitsForm" type="submit" name="visitId" value="${visit.id }" formmethod="POST" formaction="${pageContext.request.contextPath}/visit/patient/removeVisit"  onclick="return confirmRemoveVisit()">Odwołaj wizytę</button>
						</c:if>
					</td>
				</tr>
					<c:forEach items="${visit.visitTreatmentComment }" var="treat" varStatus="loop2">
						<c:if test="${treat.treatment.id != 1 }">
							<tr>
								<td>${loop.count}.${loop2.count}</td>
								<td colspan="3"></td>
								<td>${treat.treatment.name}</td>
								<td>${treat.treatment.price}</td>
							</tr>
						</c:if>
					</c:forEach>			
			</tbody>
